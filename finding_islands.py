...
def find_tables(sheet: list[list[int]]) -> list[list[int]]:
    """
    ...
    for row in range(len(sheet)):
        for col in range(len(sheet[0])):
            if sheet[row][col] == 1 and checked[row][col] == 0:
                checked[row][col] = 1
                b_row, b_col = bfs(sheet, (row, col), checked)
                table_coords.append([row, col, b_row, b_col])

    return table_coords

def bfs(sheet: list[list[int]], root:tuple, checked:list[list[int]]) -> tuple[int, int]:
    """
    ...
    while len(search_queue):
        ...
        search_queue.extend(find_connected_nodes(sheet, curr_node, checked))
    
    return bottom_right

def find_connected_nodes(sheet: list[list[int]], curr_node:tuple, checked:list[list[int]]) -> list[tuple[int, int]]:
    """
